(function (exports) {
	...
	Validator.prototype.add = function(schema, schemaId) {
	...
	Validator.prototype.validate = function(object, schema, options) {
		if (typeof schema !== 'object') {
			this.currentSchemaId = schema;
			schema = this._get(schema);
		} else {
			this.currentSchemaId = undefined;
		}

		this.errors = [];
		this.currentOptions = mixin({}, this.options, options || {});
		try {
			this.validateObject(object, schema);
		} catch(err) {
			if (this.currentOptions.failOnFirstError || !(err instanceof ValidatorError)) {
				throw err;
			}
		}
	...
	Validator.prototype.validateObject = function (object, schema) {
		...
		if (schema.additionalProperties === undefined) {
			schema.additionalProperties = this.currentOptions.additionalProperties;
		}
		// see 5.4
	...
	function (object, value, property, schema, indexInArray) {
		...
		if (schema.$ref) {
			var ref = schema.$ref;
			if (ref === '#') {
				schema = this._get(this.currentSchemaId);
			} else {
				this.currentSchemaId = ref;
				schema = this._get(ref);
			}
		}

		//validate default value
		if (this.currentOptions.validateDefaultValue && schema['default'] !== undefined &&
				property != 'default') {
		...
		if (value === undefined) {
			...
			if (this.currentOptions.applyDefaultValue && schema['default'] !== undefined) {
		...
		if (this.currentOptions.cast) {
			...
			//for simple value
			if (this.currentOptions.castSource && indexInArray == null && object[property] !== value) {
				object[property] = value;
			} else if (this.currentOptions.castSource && indexInArray != null && object[property][indexInArray] !== value) {
			//and for an array
				object[property][indexInArray] = value;
			}
		}

		if (schema.format && this.currentOptions.validateFormats) {
			format = schema.format;

			if (this.currentOptions.validateFormatExtensions) { spec = this.formatExtensions[format] }
			if (!spec) { spec = Validator.formats[format] }
			if (!spec) {
				if (this.currentOptions.validateFormatsStrict) {
			...
			switch (type || (isArray(value) ? 'array' : typeof value)) {
				...
				case 'array':
					constrain('items', value, function (a, e) {
						for (var i = 0, l = a.length; i < l; i++) {
							self.validateProperty(object, a[i], property, e, i);
						}
						return true;
					});
				...
				case 'object':
					...
					if (schema.properties || schema.patternProperties || schema.additionalProperties) {
						self.validateObject(value, schema);
					}
					break;
			}
				...
				if (type != 'array' && type != 'object') {
					...
					for (var i = 0, l = filters.length; i < l; i++) {
						...
						if (isString(filters[i])) {
							if (filters[i] in self.filters) {
								filter = self.filters[filters[i]];
							} else {
								return self.error('filter', property, filters[i], schema, 'unknown filter: %{actual}');
							}
						} else if (isFunction(filters[i])) {
							filter = filters[i];
						} else {
							return self.error('filter', property, typeof filters[i], schema, 'bad filter type: %{actual}');
						}
						try {
							object[property] = filter(object[property]);
						} catch(err) {
							return self.error('filter', property, err, schema, 'error during filtering: %{actual}');
						}
					}
				} else {
					return self.error('filter', property, type, schema, 'bad property type for filtering: %{actual}');
				}
			}
		});
	};
	...
	Validator.prototype.error = function(attribute, property, actual, schema, message) {
		...
		if (this.currentOptions.exitOnFirstError || this.currentOptions.failOnFirstError) {
