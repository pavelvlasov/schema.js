(function (exports) {
	...
	Validator.prototype.validate = function (object, schemaId, options) {
		options = mixin({}, Validator.defaults, options);
		this.currentSchemaId = schemaId;
		this.currentOptions = options;
		this.errors = [];
		var schema = this._get(schemaId);
		try {
			this.validateObject(object, schema, options);
		} catch(err) {
	...
	Validator.prototype.validateObject = function (object, schema, options) {
		...
		if (schema.properties) {
			...
			for (var p in props) {
				if (props.hasOwnProperty(p)) {
					visitedProps.push(p);
					this.validateProperty(object, object[p], p, props[p], options);
				}
			}
		}
				...
				if (props.hasOwnProperty(p)) {
					...
					for (var k in object) {
						if (object.hasOwnProperty(k)) {
							visitedProps.push(k);
							if (re.exec(k) !== null) {
								this.validateProperty(object, object[k], p, props[p], options);
							}
						}
					}
				}
		...
		if (undefined !== schema.additionalProperties) {
			...
			// additionalProperties is a schema and validate unvisited properties against that schema
				...
				for (i = 0, l = unvisitedProps.length; i < l; i++) {
					this.validateProperty(object, object[unvisitedProps[i]], unvisitedProps[i], schema.unvisitedProperties, options);
				}
			}
		}
	...
	function (object, value, property, schema, options, indexInArray) {
		var self = this;
		var format,
			valid,
			spec,
		...
		if (options.validateDefaultValue && schema['default'] !== undefined &&
				property != 'default') {
			this.validateProperty(schema, schema['default'], 'default', schema, options);
		}
		...
		if (options.cast) {
			...
			if (options.castSource && indexInArray == null && object[property] !== value) {
				object[property] = value;
			} else if (options.castSource && indexInArray != null && object[property][indexInArray] !== value) {
			//and for an array
		...
		if (schema.format && options.validateFormats) {
			...
			if (options.validateFormatExtensions) { spec = Validator.formatExtensions[format] }
		...
		if (typeof schema.dependencies === 'object') {
			this.validateObject(object, schema.dependencies, options);
		}
